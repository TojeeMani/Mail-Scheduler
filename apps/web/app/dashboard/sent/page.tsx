...
export default function SentPage() {
    ...
    const fetchJobs = async () => {
        ...
        try {
            ...
            setJobs(res.data.filter((j: any) => j.status === 'SENT' || j.status === 'FAILED'));
        } catch (error) {
            console.error(error);
        } finally {
            setLoading(false);
    ...
    return (
        <div className="space-y-6">
            <h2 className="text-2xl font-bold text-gray-800">Sent History</h2>
                ...
                <div className="bg-white shadow overflow-hidden rounded-lg">
                    <table className="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                            {jobs.length === 0 ? (
                                <tr>
                                    <td colSpan={4} className="px-6 py-4 text-center text-gray-500">No sent emails found.</td>
                                </tr>
                            ) : (
                                jobs.map((job) => (
                                    <tr key={job.id}>
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                            {job.sentAt ? format(new Date(job.sentAt), 'PPpp') : '-'}
                                        </td>
